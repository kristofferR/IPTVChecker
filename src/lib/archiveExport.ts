...
const CATCHUP_ATTRS = ["catchup", "catchup-days", "catchup-source", "catchup-type", "tvg-rec", "timeshift"];
  ...
  for (const result of results) {
    ...
    if (verdict === "verified") {
      ...
      exported.push(
        days == null
          ? result
          : { ...result, catchup_days: days, extinf_line: setCatchupDays(result.extinf_line, days) },
      );
    }
  }
  return exported;
