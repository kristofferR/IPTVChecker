...
export const Toolbar = memo(function Toolbar({
  ...
  const catchupVerdictsAvailable = useMemo(
    () =>
      completedResults.some(
        (result) =>
          hasArchive(result) && archiveVerdict(result, archiveProbes[result.index]) !== "advertised",
      ),
  ...
  const verifyScopeCounts = useMemo(() => {
    ...
    return {
      ...
      selected: completedResults.filter((result) => selected.has(result.index) && hasArchive(result))
        .length,
    };
                ...
                {(
                  ...
                  >
                    ...
                    <span className="text-[11px] tabular-nums text-text-tertiary">
                      ...
                      {value === verifyScope && verifyMode === "quick" && verifyScopeCounts[value] > 0
